...
async function fetchUserInfos() {
    try {
        ...
        const responseA = await fetch('http://49.232.48.114/api/player/info', {
            method: 'GET',
            headers: {
        ...
        const responseB = await fetch('http://49.232.48.114/api/player/info', {
            method: 'GET',
            headers: {
...
async function useCatalyst(askUid) {
    ...
    if (vipLevel < 5) {
        ...
        while (isAccelerating) {
            ...
            try {
                const useResponse = await fetch('http://49.232.48.114/api/prop/use?id=37', {
                    method: 'POST',
                    headers: {
                        'authorization': selectedAccountA.sso,
                        'Content-Type': 'application/json'
    ...
    try {
        const useResponse = await fetch('http://49.232.48.114/api/prop/use?id=37&targetId=-1&num=5', {
            method: 'POST',
            headers: {
                'authorization': selectedAccountA.sso,
                'Content-Type': 'application/json'
        ...
        if (useData.code === 500 && useData.msg === "你没有催产剂可用") {
            ...
            const retryResponse = await fetch('http://49.232.48.114/api/prop/use?id=37&targetId=-1&num=5', {
                method: 'POST',
                headers: {
                    'authorization': selectedAccountA.sso,
                    'Content-Type': 'application/json'
...
async function buyCatalyst() {
    const shopResponse = await fetch('http://49.232.48.114/api/shop/prop?id=37', {
        method: 'GET',
        headers: {
            'authorization': selectedAccountA.sso,
            'Content-Type': 'application/json'
...
async function finishBirth(askUid) {
    ...
    try {
        const response = await fetch(`http://49.232.48.114/api/birth/finish?uid=${askUid}`, {
            method: 'POST',
            headers: {
                'authorization': selectedAccountA.sso,
                'Content-Type': 'application/json'
...
async function performBreeding(bird) {
    ...
    try {
        ...
        while (hasMore) {
            const blesserBirdResponse = await fetch(`http://49.232.48.114/api/birth/birthwait?birdId=${bird.id}&page=${page}`, {
                method: 'GET',
                headers: {
                    'authorization': selectedBlesser.user.sso,
                    'Content-Type': 'application/json'
        ...
        const blessingResponse = await fetch(`http://49.232.48.114/api/birth/make?birdId=${blesserBird.id}&friendBirdId=${bird.id}`, {
            method: 'POST',
            headers: {
                'authorization': selectedBlesser.user.sso,
                'Content-Type': 'application/json'
