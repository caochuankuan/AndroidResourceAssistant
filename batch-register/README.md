...
页面要求输入一个基于北京时间日期生成的 8 位动态密码。密码由数字和大写英文字母组成，每天北京时间 `00:00` 自动变化，外观不包含可识别的日期格式。

该校验按要求只在浏览器 JavaScript 中执行，适合阻止不了解工具的访客误用，但不属于真正的安全鉴权。能够查看前端源码的用户仍可分析生成逻辑；如果需要严格限制访问，应在 Worker 中增加服务端密钥或 Cloudflare Access。
...
Cloudflare Workers 不能直接使用裸 IP 作为 `fetch` 目标。部署前需要在 `yujian.love` 的 Cloudflare DNS 中添加：

| 设置 | 值 |
|---|---|
| 类型 | `A` |
| 名称 | `api-origin` |
| IPv4 地址 | `116.62.238.93` |
| 代理状态 | `仅 DNS`（灰云） |
| TTL | 自动 |
...
5. 输入当天的每日访问密码。
6. 点击“开始批量注册”。
7. 在结果表格中实时查看基础流程和三个兑换结果。
8. 任务结束后，可重试未完成账号或导出 CSV。
